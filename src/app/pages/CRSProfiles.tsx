...
const ThemeIndicator = ({ val, label }: { val: number; label: string }) => (
  <div className="flex flex-col items-center gap-1 group/theme relative">
...
export function CRSProfiles() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (registrySearchQuery.trim()) {
      ...
      result.sort((a,b) => (b[activeMeasure] || 0) - (a[activeMeasure] || 0));
    }

    return result;
  }, [allRecords, filters, entityType, selectedEntity, activeMeasure, registrySearchQuery]);
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                   <div className="px-6 py-4 bg-slate-50/50 border-b border-slate-200 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         <h2 className="text-slate-900 font-semibold uppercase tracking-widest text-sm">Project registry</h2>
                         <span className="px-2.5 py-0.5 bg-blue-50 text-blue-700 text-[13px] font-bold rounded-full border border-blue-100">{filteredRecords.length.toLocaleString()} results</span>
                      </div>
                      <div className="flex items-center gap-6">
                         <div className="flex items-center gap-3 bg-white px-3 py-1.5 rounded-xl border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-blue-500/20 w-64">
                            ...
                            <input 
                              ...
                              onChange={(e) => { setRegistrySearchQuery(e.target.value); setPage(1); }}
                            />
                         </div>
                         <div className="flex items-center gap-1.5">
                            ...
                            >←</button>
                            <span className="text-sm font-medium text-slate-700 px-2">Page {page} of {Math.ceil(filteredRecords.length / rowsPerPage) || 1}</span>
                            <button 
                   ...
                   <div className="overflow-x-auto min-h-[400px]">
                     {recordsLoading ? (
                       ...
                       <table className="w-full border-collapse">
                         <thead>
                           <tr className="bg-slate-50/20 text-[12px] text-slate-500 font-bold uppercase tracking-wider border-b border-slate-200">
                             <th className="px-6 py-4 text-left">Year</th>
                             <th className="px-6 py-4 text-left">Project details</th>
                             <th className="px-6 py-4 text-left">Mode</th>
                             <th className="px-6 py-4 text-center">Thematic alignment (6)</th>
                             <th className="px-6 py-4 text-right">Amount (million USD)</th>
                           </tr>
                          </thead>
                             ...
                             <tr>
                               <td colSpan={5} className="px-6 py-20 text-center">
                                  <div className="flex flex-col items-center gap-3">
                                     ...
                                     <p className="text-slate-500 font-medium">No results matched your search within this entity.</p>
                                  </div>
                               </td>
                             </tr>
                           ) : (
                             pagedRecords.map((record) => (
                               <tr key={record.id} onClick={() => setActiveRecord(record)} className="hover:bg-slate-50/80 transition-colors group cursor-pointer">
                                 <td className="px-6 py-5 text-[14px] text-slate-500 tabular-nums font-medium">{record.year}</td>
             ...
             {activeRecord && (
                <div className="h-full flex flex-col">
                   <div className="bg-slate-900 p-10 text-white relative overflow-hidden">
                      ...
                      <span className="px-3 py-1 bg-blue-600 rounded-full text-[12px] font-bold uppercase tracking-widest shadow-lg shadow-blue-900/50">Project detail review</span>
                   ...
                   <div className="flex-1 overflow-y-auto p-10 space-y-10">
                      ...
                      <div className="space-y-6">
                         <h3 className="text-[14px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-3">Institutional Identification</h3>
                         <div className="grid grid-cols-2 gap-y-8 gap-x-8">
                            <div>
                               <p className="text-[12px] text-slate-400 font-bold uppercase tracking-tight">Source Donor</p>
                               <p className="text-base font-bold text-slate-900">{activeRecord.donor}</p>
                            </div>
                            <div>
                               <p className="text-[12px] text-slate-400 font-bold uppercase tracking-tight">Reporting Agency</p>
